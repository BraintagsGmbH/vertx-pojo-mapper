 ...
 * #L%
 */
package de.braintags.io.vertx.pojomapper.mongo.testtest;
...
import de.braintags.io.vertx.pojomapper.mongo.dataaccess.MongoQueryResult;
import de.braintags.io.vertx.pojomapper.mongo.vertxunit.MongoDataStoreContainer;
...
public class TestAsAsync {
  private static final io.vertx.core.logging.Logger logger = io.vertx.core.logging.LoggerFactory
      .getLogger(TestAsAsync.class);
  ...
  public static void setUp() {
    vertx = Vertx.vertx();
    System.setProperty("IDatastoreContainer", MongoDataStoreContainer.class.getName());
  }
    ...
    write.save(result -> {
      if (result.failed()) {
        ...
        query.execute(qr -> {
          ...
          try {
            MongoQueryResult<ReferenceMapper_Array> mqr = (MongoQueryResult<ReferenceMapper_Array>) qr.result();
            logger.info(mqr.getOriginalQuery().toString());
            logger.info(mqr.getOriginalResult().toString());
            logger.info("size: " + qr.result().size());

            mqr.iterator().next(itr -> {
              if (itr.failed()) {
                err.setThrowable(itr.cause());
              } else {
