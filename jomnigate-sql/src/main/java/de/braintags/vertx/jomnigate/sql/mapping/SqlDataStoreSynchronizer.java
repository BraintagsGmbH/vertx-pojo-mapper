...
public class SqlDataStoreSynchronizer extends AbstractDataStoreSynchronizer<String> {
   ...
   */
  public SqlDataStoreSynchronizer(final SqlDataStore ds) {
    this.datastore = ds;
  }
  ...
  private void readTableFromDatabase(final IMapper mapper, final Handler<AsyncResult<SqlTableInfo>> resultHandler) {
    ...
    SqlUtil.query(datastore, tableQuery).setHandler(result -> {
      if (result.failed()) {
        resultHandler.handle(Future.failedFuture(result.cause()));
      } else {
        ...
        SqlUtil.query(datastore, columnQuery).setHandler(colResult -> readColumns(tInfo, colResult, resultHandler));
      }
    });
  }
