...
@Indexes(@Index(name = "testindexMiniMapper", fields = {
    @IndexField(fieldName = "name") }, options = @IndexOptions(unique = true)))
public class MiniMapperIndexUnique {
  @Id
  ...
  public MiniMapperIndexUnique() {
  }

  public MiniMapperIndexUnique(String name) {
    this.name = name;
  }
