...
This method uses the `link:unavailable[Properties]`, which are defined inside the
`link:unavailable#get_properties-instance_method[getProperties]` to create a suitable IDataStore.
The possible properties to be set are defined inside the appropriate implementation of
`link:unavailable[IDataStoreInit]`, please refer to the documentation there.
...
----
This method loads the `link:unavailable[DataStoreSettings]` from a local file from a Json
format directly
Michael Remme
...
pure in memory counter, which starts at zero with every new `link:unavailable[IDataStore]`,
...
`link:unavailable#get_default_key_generator-instance_method[getDefaultKeyGenerator]`. As a value for this annotation you are
defining the name of the IKeyGenerator, which shall be used, for instance
`link:todo[FileKeyGenerator.NAME]`. You must define an IKeyGenerator,
which is supported by the `link:unavailable[IDataStore]`, otherwise an Exception will be thrown
